...
// Then load Vela-specific environment variables from plugin-solana root
...
async function startVela() {
  try {
    ...
    try {
        const { TreasuryAgent } = await import("./agents/treasury/TreasuryAgent.ts");
        const velaAgent = new TreasuryAgent(solanaRuntime);
        await velaAgent.initialize();
        elizaLogger.info("Vela agent initialized");
    } catch (error) {
        ...
        throw new Error(`Failed to initialize Vela agent: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
