...
export class SqlJsDatabaseAdapter extends DatabaseAdapter<Database> implements IDatabaseCacheAdapter {
    db: Database;
  ...
  AND roomId = ?`;
    ...
    async query(sql: string, params?: Record<string, any>): Promise<any[]> {
        const results = this.db.exec(sql, params as ParamsObject);
