    ...
    async loadClients(): Promise<void> {
        for (const client of this.config.clients) {
            try {
                ...
                console.error(`Error loading client ${client}:`, error);
            }
        }
    }
    ...
    async loadEvaluators(): Promise<void> {
        for (const evaluator of this.config.evaluators) {
            try {
                ...
                console.log(`Loaded evaluator: ${evaluator}`);
            } catch (error) {
                console.error(`Error loading evaluator ${evaluator}:`, error);
            }
        }
    }
...
export class AgentRuntime implements IAgentRuntime {
    ...
    registerAction(action: Action) {
        ...
        actionRegistry.registerRuntimeAction(action, this.agentType);
    ...
    async processActions(
        ...
        for (const response of responses) {
            ...
            let action = actionRegistry.getRuntimeAction(normalizedAction);

            // Fallback to local actions for backward compatibility
            if (!action) {
