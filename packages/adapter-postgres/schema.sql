...
-- Create memories_versions table
...
-- Create indexes for memories_versions
CREATE INDEX IF NOT EXISTS idx_memories_versions_id ON memories_versions(id);
CREATE INDEX IF NOT EXISTS idx_memories_versions_created ON memories_versions(created_at);

-- Create unique constraint for user profiles
CREATE UNIQUE INDEX IF NOT EXISTS idx_unique_user_profile ON memories ((content->>'type'), (content->>'userId'))
WHERE content->>'type' = 'user_profile';
