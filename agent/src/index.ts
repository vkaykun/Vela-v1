...
function initializeFsCache(baseDir: string, character: Character) {
    ...
    const cache = new CacheManager(new FileCacheAdapter(cacheDir));
    return cache;
}
    ...
    switch (cacheStore) {
        case CacheStoreType.REDIS:
            if (process.env.REDIS_URL) {
                ...
                if (!character?.id) {
                    throw new Error(
                        "CacheStoreType.REDIS requires id to be set in character definition"
                    );
                }
        ...
        case CacheStoreType.DATABASE:
            if (db) {
                elizaLogger.info("Using Database Cache...");
                return initializeDbCache(character, db);
            } else {
                throw new Error(
                    "Database adapter is not provided for CacheStoreType.Database."
                );
            }

        case CacheStoreType.FILESYSTEM:
            elizaLogger.info("Using File System Cache...");
            if (!baseDir) {
                throw new Error(
                    "baseDir must be provided for CacheStoreType.FILESYSTEM."
                );
            }
...
async function startAgent(
    ...
    try {
        ...
        const cache = initializeCache(
            process.env.CACHE_STORE ?? CacheStoreType.DATABASE,
            character,
            "",
            db
        );
